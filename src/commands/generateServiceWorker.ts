...
export async function generateServiceWorker(): Promise<void> {
  ...
  if (await fileExists(defaultSwPath)) {
    ...
    if (!existingContent.includes('Generated by custom-push')) {
      suggestedPath = `public/${SERVICE_WORKER_FALLBACK_FILENAME}`
      logger.warn(`⚠  ${SERVICE_WORKER_FILENAME} already exists and was not generated by custom-push.`)
  ...
  try {
    ...
    logger.error('   Try reinstalling custom-push: npm install -g custom-push')
    return
  }
  ...
  try {
    ...
    logger.info(`  1. Install package: npm install custom-push`)
    logger.info(`  2. Wrap your app: import { CustomPushProvider } from 'custom-push'`)
